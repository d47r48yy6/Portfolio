import React from 'react'
import { Spotlight } from './ui/Spotlight'
import { TextGenerateEffect } from './ui/TextGeneratorEffect'
import { MagicButton } from './ui/MagicButton'

const Hero = () => {
  return (
    <div className='pb-20 pt-36'>
        <div>
             {/*Made The Page Responsive for the Three Described Screen Size  */}
            <Spotlight className='-top-40 -left-10 md:-left-32 md:-top-20 h-screen'fill='white' />
            <Spotlight className='top-10 left-full h-[80vh] w-[50vw] 'fill='purple' />
            <Spotlight className='top-28 left-80 h-[80vh] w-[50vw]'fill='blue' />
            
        </div>
        {/* Background Grids and dots in the page */}
        <div className="h-screen w-full dark:bg-black-100 bg-white  dark:bg-grid-white/[0.03] bg-grid-black/[0.02] flex items-center justify-center absolute top-0 left-0">
      {/* Radial gradient for the container to give a faded look */}
      <div className="absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black-100 bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]" />
    </div>
    <div className='flex= justify-center relative my-20'>
      <div className='max-w-[89vw] md:max-w-2xl lg:max-w-[60vw] flex flex-col items-center justify-center '>
        <h2>
          Dynamic web Magic with next.js
        </h2>
        <TextGenerateEffect 
        className='text-center text-40[40px] md:text-5xl lg:text-6xl'
        words='Trasforming Concepts into Seamless Experiences '/>
        <p className='text-center md:tracking-wider mb-4 text-sm md:text-lg lg:text2xl'>
          Hi , I&apos; am Ayush Yadav a Software Developer
        </p>
        <MagicButton />
      </div>
    </div>

    </div>
  )
}

export default Hero